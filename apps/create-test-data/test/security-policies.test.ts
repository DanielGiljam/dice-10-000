...
const tableMethodMap = {
  ...
  rolls: {
    select: async (builder: SupabaseQueryBuilder<any>) =>
      await builder
        .select()
        .eq('id', gamesWithEverything[0].rounds[0].turns[0].rolls[0].id),
    /* insert: async (builder: SupabaseQueryBuilder<any>) =>
        await builder.insert({
          name: `${capitalize(faker.word.adjective())} ${capitalize(
            faker.word.adjective()
...
beforeAll(async () => {
  ...
  if (fs.existsSync(testDataPath)) {
    ...
    const gamesWithPlayers1 = await createGames(supabase, players.slice(0, 3), {
      count: 1,
    });
    const [gameWithEverything1] = await playGames(supabase, gamesWithPlayers1);
    const gamesWithPlayers2 = await createGames(supabase, players.slice(2), {
      count: 1,
    });
    const [gameWithEverything2] = await playGames(supabase, gamesWithPlayers2);
...
(
  ...
  describe(actor, () => {
    ...
    Object.entries(tableMethodMap).forEach(([table, methodMap]) => {
      Object.entries(methodMap).forEach(([method, testImplementation]) => {
        const canOrCannot = canOrCannotMap[table][method] ?? 'cannot';
        describe(`${canOrCannot} ${method} ${table}`, () => {
          let result;
          beforeAll(async () => {
            result = await testImplementation(client.from(table));
          });
          if (method === 'select') {
            ...
            if (canOrCannot === 'can') {
              describe('the row was actually deleted', () => {
                let result;
                beforeAll(async () => {
                  result = await methodMap['select'](serviceClient.from(table));
                  console.log(
